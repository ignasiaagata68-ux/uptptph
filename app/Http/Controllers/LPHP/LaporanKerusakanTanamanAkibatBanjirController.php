    ...
    public function editPerbaikan($id)
{
    ...
    $detailModel = DetLaporanKerusakanTanamanAkibatBanjir::findOrFail($id);

    $header = LaporanKerusakanTanamanAkibatBanjir::where(
            'id_laporan_kerusakan_tanaman_akibat_banjir',
            $detailModel->id_laporan_kerusakan_tanaman_akibat_banjir
        )
        ->whereIn(
            'id_kabupaten_kota',
            $wilayah
    ...
    $idDetail = $detailModel->id_det_laporan_kerusakan_tanaman_akibat_banjir;
...
public function updatePerbaikan(Request $request, $id)
{
    ...
    // Validasi bahwa detail ini benar-benar milik wilayah LPHP
    $header = LaporanKerusakanTanamanAkibatBanjir::where(
            'id_laporan_kerusakan_tanaman_akibat_banjir',
    ...
    $detail->update([
        ...
        'luas_tanam'            => $this->decimal($request->luas_tanam[0]),
        'luas_waspada'          => $this->decimal($request->luas_waspada[0]),

        'sps_surut_luas'        => $this->decimal($request->sps_surut_luas[0]),
        'sps_surut_ket'         => $request->sps_surut_ket[0],

        'sps_puso_luas'         => $this->decimal($request->sps_puso_luas[0]),
        'sps_puso_ket'          => $request->sps_puso_ket[0],

        'luas_tambah_terkena'   => $this->decimal($request->luas_tambah_terkena[0]),
        'luas_tambah_puso'      => $this->decimal($request->luas_tambah_puso[0]),

        'luas_keadaan_terkena'  => $this->decimal($request->luas_keadaan_terkena[0]),
        'luas_keadaan_puso'     => $this->decimal($request->luas_keadaan_puso[0]),

        'penangan_upaya'        => $request->penangan_upaya[0],
        'penangan_jumlah'       => $this->decimal($request->penangan_jumlah[0]),

        'koordinat_latitude'    => $this->decimal($request->koordinat_latitude[0]),
        'koordinat_longitude'   => $this->decimal($request->koordinat_longitude[0]),
